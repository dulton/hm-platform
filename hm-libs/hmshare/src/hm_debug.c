...
hm_debug_get_sub_str(const gchar *start, const gchar *end, 
...
static __inline__ gint
hm_debug_create_path(const gchar *path, mode_t mode, gchar abs_p[])
{
    ...
    while ( TRUE )
    {
        ...
        if (hm_debug_get_sub_str(start, end, 
...
static __inline__ gint
hm_debug_create_log_file(const gchar *file_folder, const gchar *name)
{
    gint err;
    time_t time_now;
    struct tm tm_now;
    ...
    if ((err = hm_debug_create_path(file_folder,
...
FILE *
hm_debug_get_log_file( void )
{
    if (G_UNLIKELY(!log_file))
        return stderr;

    hm_debug_check_rewind();
...
hm_debug_get_level_str(GLogLevelFlags log_level)
{
    switch (log_level)
    {
...
static void
hm_debug_log_assertion(const gchar *msg)
{
    FILE *fp;

    fp = hm_debug_get_log_file();
...
static void
hm_debug_log_to_file(const gchar *log_domain, GLogLevelFlags log_level,
    const gchar *msg, gpointer user_data)
{
    ...
    fp = hm_debug_get_log_file();
    level = hm_debug_get_level_str(log_level);
...
static __inline__ void
hm_debug_log_set_exit_handler( void )
{
    on_exit(hm_debug_on_exit, NULL);
}


__export void
hm_debug_set_log_size(gint size)
{
...
static __inline__ gint
hm_debug_set_fp_nonblock(FILE *fp)
{
	gint fd, old_flgs;
...
static __inline__ void
hm_debug_set_fps_nonblock( void )
{
	hm_debug_set_fp_nonblock(stdout);
	hm_debug_set_fp_nonblock(stderr);
	hm_debug_set_fp_nonblock(log_file);
}
