...
static __inline__ XmlData *alloc_one_xml_data(xmlid_t id, size_t size)
{
	...
	if (NULL != (msg = hm_xml_alloc(sizeof(XmlData) + size)))
	{
		msg->id = id;
...
static __inline__ void xml_data_destroy(XmlData *xml_data)
{
	...
	hm_xml_dealloc(xml_data, xml_data->size);
}
	...
	int data_id, size_t data_size, parse_func func)
{
	...
	pri_data = hm_xml_alloc(data_size);
	ret = (*func)(pri_data, new_buf);
	
	if (1 == flag)
		hm_xml_dealloc(new_buf, buf_size+1);
	
	if (0 == ret)
	{
		msg = jpf_xml_msg_new(data_id, pri_data, data_size);
	}
	hm_xml_dealloc(pri_data, data_size);
	...
	int data_id, size_t data_size, parse_func func, JpfXmlMsgDes fin)
{
	...
	flag = get_xml_buf(&new_buf, page_buf, buf, buf_size);
	xml_data = hm_xml_alloc(data_size);
...
static void destory_user_list_info(void *pri_data, size_t pri_size)
{
	...
	if (user_list->count)
	{
		hm_xml_dealloc(user_list->user_info, 
			user_list->count * sizeof(JUserInfo));
	}
	hm_xml_dealloc(user_list, sizeof(struct _prx_user_st));
...
static void destory_device_list_info(void *pri_data, size_t pri_size)
{
	...
	if (dev_list->count)
	{
		hm_xml_dealloc(dev_list->device_info, 
			dev_list->count * sizeof(struct _prx_device_info));
	}
	hm_xml_dealloc(dev_list, sizeof(struct _prx_device_info));
...
static void destory_factory_list_info(void *pri_data, size_t pri_size)
{
	...
	{
		hm_xml_dealloc(factory_list->factory, 
			factory_list->count * sizeof(struct _prx_factory_info));
	}
	hm_xml_dealloc(factory_list, sizeof(struct _prx_factory_info));
...
static void destory_transparent_param(void *priv_obj, size_t size)
{
	...
	if (trans->data && trans->length)
		hm_xml_dealloc((void*)trans->data, trans->length);
	hm_xml_dealloc((void*)trans, sizeof(TransparentPacket));
}
		...
		size_t buf_size, int data_id, parse_func func)
{
	...
	flag = get_xml_buf(&new_buf, page_buf, buf, buf_size);
	tran_packet = hm_xml_alloc(sizeof(TransparentPacket));
