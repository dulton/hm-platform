...
static __inline__ void jpf_priv_obhm_destroy(void *priv, size_t size)
{
	J_ASSERT(priv && size);
	
	return hm_xml_dealloc(priv, size);
}
		...
		size_t priv_size, JpfXmlMsgDes destroy)
{
	...
	msg = hm_xml_alloc(sizeof(JpfXmlMsg));
...
JpfXmlMsg* jpf_xml_msg_new(xmlid_t id, void *priv_obj, size_t priv_size)
{
	...
	{
		copy_obj = hm_xml_alloc(priv_size);
		memcpy(copy_obj, priv_obj, priv_size);
		destroy = jpf_priv_obhm_destroy;
	}
...
void jpf_xml_msg_destroy(JpfXmlMsg *self)
{
	...
	return hm_xml_dealloc(self, sizeof(JpfXmlMsg));
}
	...
	char cmd_buf[], size_t cmd_size)
{
	...
	if (1 == ret)
		hm_xml_dealloc(new_buf, xml_size+1);
