...
static int xml_tree_node_free(XmlTreeNode *xml_node, int flags)
{
	...
	while (NULL != xml_node)
	{
		...
		while (NULL != tree_attr)
		{
			...
			hm_xml_dealloc(tree_attr, sizeof(XmlTreeAttr));
			tree_attr = p_attr;
		}
        ...
        if (xml_node->element.extend_contact)
        {
            ...
            hm_xml_dealloc(xml_node->element.extend_contact, size);
        }
        
		hm_xml_dealloc(xml_node, sizeof(XmlTreeNode));
		xml_node = p_xml_node;
	}
	...
	mxml_node_t *mxml_node, int flags)
{
	...
	{
		tree_attr = (XmlTreeAttr*)hm_xml_alloc(sizeof(XmlTreeAttr));
										...
										mxml_node_t *mxml_node_parent, int flags)
{
	...
	while (NULL != mxml_node_child)
	{
		...
		{
			p_tree_node = (XmlTreeNode *)hm_xml_alloc(sizeof(XmlTreeNode));
