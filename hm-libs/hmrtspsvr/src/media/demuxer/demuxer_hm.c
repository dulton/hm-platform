 ...
 *
 * Copyright (C) 2012 HiMickey <bugs@himickey.com>
 * See COPYING for more details
 *
 * rtsp-server is a stream transporting and controlling module using
 * RTP/RTCP/RTSP protocols, which is designed for HiMickey platform servers
...
{
	if (!hmf_file_ops || !hmf_file_ops->probe)
		return -EINVAL;

	return (*hmf_file_ops->probe)(filename);
}


static gint file_open(char *mrl, struct file_ctx *filp_ctx)
{
	if (!hmf_file_ops || !hmf_file_ops->open)
		return -EINVAL;

	return (*hmf_file_ops->open)(mrl, filp_ctx);
}


static void file_close(struct file_ctx *filp_ctx)
{
	if (!hmf_file_ops || !hmf_file_ops->close)
		return;

	(*hmf_file_ops->close)(filp_ctx);
}
...
{
	if (!hmf_file_ops || !hmf_file_ops->read)
		return -EINVAL;

	return (*hmf_file_ops->read)(ctx, pkt);
}


static void file_free_packet(struct file_packet *pkt)
{
	if (!hmf_file_ops || !hmf_file_ops->free_packet)
		return;

	(*hmf_file_ops->free_packet)(pkt);
}


static gint hmf_probe(const char *filename)
{
...
static gint hmf_stm_pub_init(Resource *r, struct file_ctx *fp_ctx)
{
	...
	for (stm = 0; stm < fp_ctx->streams; ++stm)
	{
		...
		case STM_VIDEO:
			...
			strcpy(props_hints.encoding_name, "HMV");
		...
		case STM_AUDIO:
			...
			trackinfo.rtp_port = 0;
			trackinfo.id = stm;
			props_hints.payload_type = 8;	/* Maybe changed in hma_init() */

			strcpy(props_hints.encoding_name, "HMA");
...
static gint hmf_init(Resource *r)
{
	struct file_ctx *ctx;
...
static gint hmf_read_packet(Resource * r)
{
	struct file_ctx *ctx;
	struct file_packet pkt;
	TrackList tr_it;
...
static gint hmf_seek(Resource * r, double time_sec)
{
	return 0;
}


static void hmf_uninit(gpointer rgen)
{
