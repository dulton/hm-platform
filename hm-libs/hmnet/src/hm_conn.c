...
__hm_connection_set_flags(JConnection *conn, int flgs)
{
    int old_flgs;
...
hm_connection_new(struct sockaddr *sa, unsigned flags, int *errp)
{
	JConnection *conn;
	...
	conn = hm_new0(JConnection, 1);	/* hmlib.has its own OOM facility */
	conn->flags = flags;
...
new_conn_failed:
	...
	hm_del(conn, JConnection, 1);
	return NULL;
}


__export int
hm_connection_listen(JConnection *conn)
{
	J_ASSERT(conn != NULL);
...
__export int
hm_connection_connect(JConnection *conn, struct sockaddr *sa)
{
	int err;
	J_ASSERT(conn != NULL);
...
__export int
hm_connection_get_fd(JConnection *conn)
{
	J_ASSERT(conn != NULL);
...
__export int
hm_connection_set_flags(JConnection *conn, unsigned flgs)
{
	int ret;
	J_ASSERT(conn != NULL);
...
__export int
hm_connection_is_blocked(JConnection *conn)
{
	J_ASSERT(conn != NULL);
...
__export JConnection *
hm_connection_accept(JConnection *listen, int *errp)
{
	JConnection *conn;
	int ret;
...
__export int
hm_connection_is_ingrogress(JConnection *conn, int clear)
{
	J_ASSERT(conn != NULL);
...
__export int
hm_connection_read(JConnection *conn, char buf[], size_t size)
{
	int ret;
...
__export int
hm_connection_write(JConnection *conn, char *buf, size_t size)
{
	size_t left = size;
	int ret;
...
__export int
hm_resolve_host(struct sockaddr_in *sin, char *host, int port)
{
...
__export int
hm_connection_get_timeout(JConnection *conn)
{
	J_ASSERT(conn != NULL);
...
__export void
hm_connection_set_timeout(JConnection *conn, int millisec)
{
	int max_timeout;
	J_ASSERT(conn != NULL);
...
__export void
hm_connection_close(JConnection *conn)
{
	J_ASSERT(conn != NULL);
...
__export char *
hm_connection_get_host(JConnection *conn, char *ip)
{
	unsigned char *sa;
...
__export char *
hm_connection_get_peer(JConnection *conn, char *ip)
{
	unsigned char *sa;
	struct sockaddr_in addr;
	socklen_t len;
