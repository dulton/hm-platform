...
hm_net_buf_init(JNetBuf *buff, size_t size, int n_blocks,
	JNetBufFlush flush)
{
	memset(buff, 0, size);
...
*/
JNetBuf *
hm_net_buf_alloc(int n_blocks, JNetBufFlush flush)
{
    JNetBuf *buf;
    size_t size;
	...
	buf = (JNetBuf*)hm_alloc(size);	/* hmlib.has its own OOM facility */
	hm_net_buf_init(buf, size, n_blocks, flush);
...
hm_net_buf_last_pos(JNetBuf *buff)
{
    JBufBlock *b;
    int last;
    J_ASSERT(buff != NULL);
...
static __inline__ void
hm_net_buf_write_ok(JNetBuf *buff)
{
...
static __inline__ void
hm_net_buf_flush_ok(JNetBuf *buff)
{
    JBufBlock *b;
...
*/
int
hm_net_buf_flush(JNetBuf *buff, void *user_data)
{
    int ret;
    J_ASSERT(buff != NULL);

    hm_mutex_lock(buff->buf_lock);
    ret = __hm_net_buf_flush(buff, user_data);
    hm_mutex_unlock(buff->buf_lock);
...
*/
int
hm_net_buf_write(JNetBuf *buff, char *buf, size_t count,
	void *user_data, int *pending)
{
    int ret;
